...
type FlavorFormValue = z.infer<typeof formSchema>;

const FlavorForm = ({ initialData }: FlavorFormProps) => {
  ...
  const form = useForm<FlavorFormValue>({
  ...
  const onSubmit = async (data: FlavorFormValue) => {
    try {
      setLoading(true);
  ...
  const onDelete = async () => {
    try {
      ...
      router.refresh();
      router.push(`/${params.storeId}/flavors`);
      toast({ title: "Flavor deleted", position: "top", status: "success" });
    } catch (error) {
      toast({
        title:
          "Make sure you removed all products that's using this flavor first.",
        status: "error",
        position: "top",
      });
    } finally {
