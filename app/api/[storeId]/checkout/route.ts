...
export async function POST(
  ...
  const session = await stripe.checkout.sessions.create({
    ...
    success_url: `${process.env.FRONTEND_STORE_URL || process.env.FRONTEND_GYMRATS_URL}/cart?success=1`,
    cancel_url: `${process.env.FRONTEND_STORE_URL || process.env.FRONTEND_GYMRATS_URL}/cart?canceled=1`,
    metadata: {
      orderId: order.id,
    },
  });
